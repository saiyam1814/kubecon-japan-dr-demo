...
This guide is the **one-time setup**, run once while online, days before the
event. It builds everything the demos need. The demos themselves - and the
reset between practice runs - live in [`DEMO-STEPS.md`](DEMO-STEPS.md).
...
## 15. Optional: prepare the vind drill cluster (for Demo 4)

Skip this section if you will not show the optional Demo 4.
...
Note: this section switches the default vcluster driver to docker; revert with
`vcluster use driver helm` after the event.

---

Setup is complete. Everything from here on - running the demos and resetting
between practice runs - lives in [`DEMO-STEPS.md`](DEMO-STEPS.md).
